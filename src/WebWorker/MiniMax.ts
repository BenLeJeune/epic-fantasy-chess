...
const miniMax = (g : Game, depth : number, maximising : boolean, army: number[], hashGet:(b:number[])=>number|null, hashSet:(b:number[],e:number)=>void, alpha:number = -Infinity, beta:number = Infinity ) => {
    ...
    let orderedPromotionMoves = unorderedMoves.filter( m => m.additional?.promotionTo )
        .sort(( p, n ) =>
            (Piece.getPiece(n.additional?.promotionTo||1)?.engineValue || 0) - ( Piece.getPiece(p.additional?.promotionTo||1)?.engineValue || 0 ))

    let orderedCaptures = unorderedMoves.filter( m => g.getBoard()[m.move.to] !== 0 )
        .sort(( p, n ) => (Piece.getPiece( g.getBoard()[n.move.to] )?.engineValue || 0) - (Piece.getPiece( g.getBoard()[p.move.to] )?.engineValue || 0) )
    ...
    if ( depth === 0 || isCheckMate ) {
        ...
        for ( let { move: m, additional } of legalMoves ) {
            ...
            let ev : number = hashedEval || miniMax(g, depth - 1, !maximising, army, hashGet, hashSet, alpha, beta)[0];
            if (!hashedEval) {
                hashSet(g.getBoard(), ev);
            }
        ...
        //         g.Move( m.from, m.to, m.special, additional );
        //         let hashedEval
        //         let ev = hashGet( g.getBoard() ) || miniMax(g, depth - 1, !maximising, army, hashGet, hashSet, alpha, beta)[0];
        //         g.UnMove()
        //         if (value < ev) {
        //             value = ev;
        //             move = { move : m, additional };
